...
public class AStarGrid : MonoBehaviour {
    ...
    public Tilemap collidableMap;
    public GameObject collidableMapGO;
	...
	void Start () {
        ...
        //Génération à partir d'une TileMap
        if (SceneManager.GetActiveScene().name == "map")
        {
            collidableMap = GameObject.Find("Walls").GetComponent<Tilemap>();
        }

        //Génération à partir d'un raycast
        else if (SceneManager.GetActiveScene().name == "Main")
        {
            collidableMapGO = GameObject.Find("BoardHolder");
            GameObject.Find("GameManager").GetComponent<BoardCreator>().gridUp = true;
	...
	void BuildGrid() {
		...
		for (int x = 0; x < gridX; x++) {
			for (int y = 0; y < gridY; y++) {
                ...
                {
                    RaycastHit2D hit = Physics2D.Raycast(checkPos, Vector2.zero);
                    if (hit.collider != null && hit.collider.name.Contains("mur"))
                    {
                        isSolid = true;
                        // hit = null;
                    }
